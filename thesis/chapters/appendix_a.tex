...
SVM                 & $<1$ second (RBF) & $\sim$3 minutes (linear SGD) \\
Random Forest       & $\sim$9 seconds   & $\sim$3 seconds \\
XGBoost             & $\sim$25 seconds  & $\sim$2.5 minutes \\
\midrule
\textbf{Total}      & $<1$ minute       & $\sim$9 minutes \\
\bottomrule
\end{tabular}
\end{table}

The unexpectedly short grid-search times on Dataset~1 are explained by the small effective sample size after deduplication ($\sim$300 unique records) combined with multi-core parallelism. On Dataset~2, training times scale roughly with the number of fits and the complexity of each model.
